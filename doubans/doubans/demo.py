...
def spider(pages, max_change_porxies_times=300):
    ...
    for i in range(0, pages):
        ...
        while True:
            ...
            time.sleep(random.uniform(3, 7))  # 每读取一次页面暂停一会,否则会被封
...
def check_proxies(http, host, port, test_url='https://book.douban.com/subject/1001103/'):
    """
    检测给定的ip信息是否可用
    ...
    for data in datas:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                with open('ips_pool.csv', 'a+') as f:
                    f.write(','.join([http, host, port]) + '\n')
                with open('url_list.txt', 'a+') as uf:
